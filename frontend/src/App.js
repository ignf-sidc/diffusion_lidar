...
import { zoom_to_multi_polygon, zoom_to_polygon } from "./hook/useZoom";
import {
...
class App extends Component {
  ...
  quit_pointer_move_dalleMenu = (id_dalle) => {
    // on parcours la liste des dalles
    this.vectorSourceGridDalle.getFeatures().forEach((feature) => {
      // son recupere la feature avec la meme id que la dalle survolé dans le menu
      if (feature.values_.properties.id === id_dalle) {
  ...
  get_dalle_in_polygon = (feature, id, type) => {
    ...
    if (this.state.dalles_select.length >= this.limit_dalle_select) {
      if (emprise == "polygon") {
        ...
        remove_dalle_menu(
          ...
          this.dalles_select,
          this.vectorSourceGridDalle,
          this.list_dalle_in_polygon
        );
      }
      message.error(
        `le nombre de dalles séléctionnées dépasse ${this.limit_dalle_select} km²`
      );
      return false;
    }
    return true;
  ...
  generate_multipolygon_bloc = () => {
    axios.get(`${this.state.api_url}:8000/data/get/blocs`).then((response) => {
  ...
  componentDidMount() {
    ...
    var createMap = () => {
      ...
      zoomToClickBloc.on("select", (event) => {
        zoom_to_polygon(event.selected[0], 11, this.state.mapInstance);
      ...
      map.on("moveend", () => {
        ...
        if (view.getZoom() >= this.zoom_dispaly_dalle) {
          ...
          axios
            .get(
              `${this.state.api_url}:8000/data/get/dalles/${minX}/${minY}/${maxX}/${maxY}`
            )
            .then((response) => {
          ...
          this.handle_mode_change({ target: { value: "click" } });
          ...
          {this.state.dalles_select.map((item, index) => (
            <div className="liste_dalle inner-div" key={index}>
              ...
              <button
                className="map-icon-button"
                onClick={() =>
                  zoom_to_polygon(item, 12, this.state.mapInstance)
                }
              >
                <FaMapMarker />
              </button>
            ...
            <div key={index}>
              <div className="liste_dalle">
                ...
                <button
                  className="map-icon-button"
                  onClick={() =>
                    zoom_to_polygon(polygon, 12, this.state.mapInstance)
                  }
                >
                  <FaMapMarker />
                </button>
                    ...
                    (dalle, key) => (
                      <div className="liste_dalle" key={key}>
                        ...
                        <button
                          className="map-icon-button"
                          onClick={() =>
                            zoom_to_polygon(dalle, 12, this.state.mapInstance)
                          }
                        >
                          <FaMapMarker />
                        </button>
